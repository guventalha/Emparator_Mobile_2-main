  ...
  const authContext = React.useMemo(
    () => ({
      signIn: async ({mail, password}) => {
        ...
        const response = await axios.get(`https://api.emparator.com/api/login?username=${mail}&password=${password}&isMobile=${true}`)
      ...
      signUp: async ({email, password, passwordConfirm}) => {
        if(password != passwordConfirm){
          Alert.alert("emparator.com", "Şifre ve şifre tekrarı aynı olmalıdır.")
        }else {
          const response = await axios.post("https://api.emparator.com/Mobile/Register",{
            Email : email,
            Password : password,
          ...
          if(response.data === "Kayıt başarılı"){
            Alert.alert("emparator.com", "Kayıt işleminiz başarıyla gerçekleşmiştir. E-postanıza gelen maili onayladıktan sonra giriş yapabilirsiniz.")
          }else if(response.data === "Bu e-posta adresi sistemimizde kayıtlıdır. Lütfen giriş sayfasından sisteme giriş yapınız veya farklı bir e-posta adresi ile kayıt olunuz."){
            Alert.alert("emparator.com","Bu e-posta adresi sistemimizde kayıtlıdır. Lütfen giriş sayfasından sisteme giriş yapınız veya farklı bir e-posta adresi ile kayıt olunuz.")
