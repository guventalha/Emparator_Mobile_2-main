...
const SearchTD = ({navigation, route}) => {
  ...
  let minFav = route.params?.MinFav ?? '';
  let maxFav = route.params?.MaxFav ?? '';
  let minSellerCount = route.params?.MinSellerCount ?? '';
  let maxSellerCount = route.params?.MaxSellerCount ?? '';
  let selectedCategories = route.params?.SelectedCategories ?? '';
  let keywords = route.params?.Keywords ?? '';

  // console.log("MİNPRICE", minPrice)
          ...
          <View
            ...
            <View>
              <Button
                title={'Filtrele'}
                name={'filter'}
                onClicked={() => navigation.navigate('trendyol.com',{
                  Keywords: keywords,
                  MinPrice: minPrice,
                  MaxPrice: maxPrice,
                  MinSale: minSale,
