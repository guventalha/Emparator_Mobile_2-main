...
const Home = (props) => {
  ...
  function getData(){
    AsyncStorage.getItem("@userName").then(response => {
      AsyncStorage.getItem("@userToken").then(token => {
        axios.post("https://api.emparator.com/Mobile/GetFrontValues",{username: response, token: token})
        .then(res => {
          if(res.data === ""){
  ...
  function renderData({item}){
    ...
    return <Popular2 data={item} onDetail={() => props.navigation.navigate("Ürün Detayı",{itemId: item.Id})}/>
  }
  ...
  return (
    <Container style={styles.container}>
      ...
      <Header style={styles.header}>
        ...
        <Body style={styles.body}>
          <Image
            ...
            style={{width:Platform.OS === 'android' ? '90%' : '75%', height:Platform.OS === 'android' ?  '48%' : '53%'}}
          />
        </Body>
      ...
      />
        ...
        <View style={{marginHorizontal: Dimensions.get('window').width * 0.04, marginTop: 5}}>
          {graphDataHistory.length>1&&
           <Chart onClick={() => props.navigation.navigate("Ürün Detayı",{itemId: graphData.Id})} image={graphData.PhotoUrl} title={graphData.Name} data={graphData} history={graphDataHistory} />
          }
        </View>
       </View>
      }  
